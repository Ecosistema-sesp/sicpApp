...
const DhOtroForm = () => {
  const route = useRoute<RouteProp<localStackNavigatorParams, 'NovedadOtro'>>();
  ...
  const Otro = useFormik({
    ...
    onSubmit: async (values: FormikValues) => {
      try {
        await axios.post('http://ecosistemasesp.unp.gov.co/sicp/api/otroreporte/', values);
  ...
  useEffect(() => {
    if (userId !== null) {
      Otro.setFieldValue('usuario_sicp', userId);
    }
  }, [userId]);
  ...
  return (
    ...
    {desdePanelNovedades ? (
      ...
      <>
        ...
        <NombreApellido setUserId={setUserId} desplazamiento={Otro} />

        <TituloContainer
          iconName="chevron-forward-circle"
          titulo="Datos y ubicación de inicio"
        />
        <FechaHora variables={{fecha : fechaInicio}} desplazamiento={Otro} tipo='fecha_inicio'/>
        <DepartamentoMunicipio desplazamiento={Otro} tipo='inicio' />
        <Ubicacion desplazamiento={Otro} tipo='inicio' />
        <TituloContainer
          iconName="checkmark-done-circle"
          titulo="descripcion"
          />
        <AreaInput placeholder='' onChangeText={(text) => Otro.setFieldValue('descripcion', text)}/>
        <BotonSubmit handleSubmit={() => Otro.handleSubmit()} buttonText="Guardar reporte" />
...
const OtroStack = createStackNavigator();
export default function OtroStackScreen() {
  const route = useRoute<RouteProp<localStackNavigatorParams, 'NovedadOtro'>>();
      ...
      options={{ 
        ...
        headerTitle: () => (
          <View style={{ width: 250}}>
              <Text>Registro otro reporte</Text>
          </View>
        ),
      }}
    />
