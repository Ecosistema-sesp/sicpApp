...
const DhFinalizacionForm = () => {
  const route = useRoute<RouteProp<localStackNavigatorParams, 'novedadFinalizacion'>>();
  ...
  const Finalizacion = useFormik({
    ...
    onSubmit: async (values: FormikValues) => {
      try {
        await axios.post('http://ecosistemasesp.unp.gov.co/sicp/api/finalizacion/', values);
  ...
  useEffect(() => {
    if (userId !== null) {
      Finalizacion.setFieldValue('usuario_sicp', userId);
    }
  }, [userId]);
  ...
  return (
    ...
    {desdePanelNovedades ? (
      ...
      <>
        ...
        <NombreApellido setUserId={setUserId} desplazamiento={Finalizacion} />

        <TituloContainer
          iconName="chevron-forward-circle"
          titulo="Datos y ubicación de inicio"
        />
        <FechaHora variables={{fecha : fechaInicio}} desplazamiento={Finalizacion} tipo='fecha_inicio'/>
        <DepartamentoMunicipio desplazamiento={Finalizacion} tipo='inicio' />
        <Ubicacion desplazamiento={Finalizacion} tipo='inicio' />
        <TituloContainer
          iconName="checkmark-done-circle"
          titulo="Motivo"
          />
        <AreaInput placeholder='' onChangeText={(text) => Finalizacion.setFieldValue('Motivo', text)}/>
        <BotonSubmit handleSubmit={() => Finalizacion.handleSubmit()} buttonText="Guardar reporte" />
...
const FinalizacionStack = createStackNavigator();
export default function FinalizacionStackScreen() {
  const route = useRoute<RouteProp<localStackNavigatorParams, 'novedadFinalizacion'>>();
  const desdePanelNovedades = route.params?.desdePanelNovedades;
  const id = route.params?.id;
  return (
  <FinalizacionStack.Navigator>
    <FinalizacionStack.Screen 
      name="DhFinalizacionForm" 
      component={DhFinalizacionForm} 
      ...
      options={{ 
        ...
        headerTitle: () => (
          <View style={{ width: 250}}>
              <Text>Registro de reporte de finalizacion anticipada de servicio</Text>
          </View>
        ),
      }}
    />
