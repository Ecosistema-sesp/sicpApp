  ...
  const NVehiculoForm = () => {
    ...
    useFocusEffect(
        React.useCallback(() => {
          const fetchData = async () => {
            try {
              ...
              const response = await fetch('http://ecosistemasesp.unp.gov.co/sicp/api/vehiculo/');
              const apiData = await response.json();
              const filteredData = apiData
              // filtar con el cronos
                .filter((item: Item) => item.usuario_sicp === userId)
                .sort((a: Item, b: Item) => Number(b.id_nvehiculo) - Number(a.id_nvehiculo));
      ...
      return (
        <Container>
          {data.map(item => ( 
            <TouchableOpacity key={item.id_nvehiculo}
            onPress={() => {     
                navigation.navigate('ResumenVehiculo', { id: item.id_nvehiculo });
            }}
>
              <View key={item.id_nvehiculo} style={{ 
                position: 'relative',
                flexDirection: 'row',
