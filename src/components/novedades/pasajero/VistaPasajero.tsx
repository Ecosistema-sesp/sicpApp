        ...
        React.useCallback(() => {
          const fetchData = async () => {
            try {
              ...
              const response = await fetch('http://ecosistemasesp.unp.gov.co/sicp/api/pasajero/');
          ...
          {data.map(item => (
            ...
            onPress={() => {  
              if (item.ubicacion_fin && item.departamentofin && item.municipiofin) {
                navigation.navigate('ResumenPasajero', { id: item.id_reporte });
              } else {
                navigation.navigate('novedadPasajero', { id: item.id_reporte, desdePanelEsquema: true });
              }
            }}
            >
              <View key={item.id_reporte} style={{ 
                ...
                ? <Icon name="people" size={55} color="#5A87C6" style={styles.icono} />
                : <Icon name="people-outline" size={55} color="#5A87C6" style={styles.icono} />
                }
