...
interface FinPasajeroProps {
    Id: Number;
    pasajero :any
    
}
const FinPasajero = ({ Id, pasajero }: FinPasajeroProps) => {
    const navigation = useNavigation<NavigationProp<RootStackParamList>>();
    const [userId, setUserId] = useState<number | null>(null);
    const [fechaFin, setFechaFin] = useState('');


    const finpasajero = useFormik({
        initialValues:{
        ...
        onSubmit: async (values: FormikValues) => {
          try {
            if (Id) {
              await axios.put(`http://ecosistemasesp.unp.gov.co/sicp/api/pasajero/${Id}/`, values);
            } else {
    ...
    let ubicacion_inicio = pasajero.values.ubicacion_inicio || 'No disponible';
      ...
      let fechaInicio = pasajero.values.fecha_inicio;
    ...
    return (
        <Container>
          ...
          <>
            ...
            <Text style={{...styles.text, marginBottom: 30}}>{pasajero.values.id_reporte}</Text>
          </>
          <TituloContainer
          ...
          <View style={styles.doble}>
            <View style={{width: '48%'}}>
              ...
              <Text style={styles.text}>{pasajero.values.fecha_inicio ? new Date(pasajero.values.fecha_inicio).toISOString().split('T')[0] : 'No disponible'}</Text>
            </View>
            <View style={{width: '48%'}}>
          ...
          <>
            ...
            <Text style={styles.text}>{pasajero.values.departamentoinicio || 'No disponible'}</Text>
          </>
          <>
            <Text style={styles.label}>Municipio:</Text>
            <Text style={{...styles.text, marginBottom: 30}}>{pasajero.values.municipioinicio || 'No disponible'}</Text>
          </>
          ...
          <View style={{marginBottom: 30}}>
            <FechaHora variables={{fecha : fechaFin}} desplazamiento={pasajero} tipo='fin'/>
            <Ubicacion desplazamiento={finpasajero} tipo='fin' />
            <DepartamentoMunicipio desplazamiento={finpasajero} tipo='fin' />
          </View>

          <BotonSubmit handleSubmit={() => finpasajero.handleSubmit()} buttonText="Finalizar dezplazamiento" />
